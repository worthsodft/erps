    ...
    public function index()
    {
        if ($this->app->request->isGet()) {
            if (AdminService::isLogin()) {
                ...
                // 登录模板
                $template = sysconf('login_template') ?: 'index';

                // 后台背景处理
                if($template == 'zsyun'){
                    $images = [
                        SystemService::uri('/static/images/login/zsyun/login_bg.jpg'),
                    ];
                    $this->logo = SystemService::uri('/static/images/login/zsyun/login_logo.png');
