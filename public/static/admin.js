...
$(function () {
    ...
    /*! 上传多张图片 */
    $.fn.uploadMultipleImage = function (is_readonly=false) {
        return this.each(function () {
            if (this.dataset.inited) return; else this.dataset.inited = 'true';
            let $bt;
            if(is_readonly) $bt = $('<div class=""><span></span><span></span></div>');
            else $bt = $('<div class="uploadimage"><span><a data-file="mul" class="layui-icon layui-icon-upload-drag"></a></span><span data-file="images"></span></div>');
            ...
            $bt.find('[data-file]').attr({
                ...
                ims.push(src), $in.val(ims.join('|')).trigger('change'), showImageContainer([src], is_readonly);
            }) && (ims.length > 0 && showImageContainer(ims, is_readonly));

            function showImageContainer(srcs, is_readonly=0) {
                $(srcs).each(function (idx, src, $img) {
                    if(is_readonly) $img = $('<div class="uploadimage uploadimagemtl"></div>');
                    else $img = $('<div class="uploadimage uploadimagemtl"><div><a class="layui-icon">&#xe603;</a><a class="layui-icon">&#x1006;</a><a class="layui-icon">&#xe602;</a></div></div>');
    ...
    $.layTable = new function () {
        this.showImage = function (image, circle, size, title) {
            ...
            option.cellExpandedMode = option.cellExpandedMode || 'tips', option.limits = params.limits || [20,50,100,200];
