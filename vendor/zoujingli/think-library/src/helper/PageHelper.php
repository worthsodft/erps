...
class PageHelper extends Helper
{
     ...
     */
    public function init($dbQuery, $page = true, bool $display = true, $total = false, int $limit = 0, string $template = '', $cusLimit=false): array
    {
        $query = $this->autoSortQuery($dbQuery);
        if ($page !== false) {
            $get = $this->app->request->get();
            $limits = [20, 50, 100, 200];
            if($cusLimit) $limits = [1,2,3,4,5,6,7,8,9,...$limits];
//            elseif($this->app->cookie->get('limit')) $this->app->cookie->delete('limit');
            if ($limit <= 1) {
                $limit = $get['limit'] ?? $this->app->cookie->get('limit', 20);
                if (in_array($limit, $limits) && ($get['not_cache_limit'] ?? 0) < 1) {
                    $this->app->cookie->set('limit', ($limit = intval($limit >= 1 ? $limit : 20)) . '');
                }
            }
