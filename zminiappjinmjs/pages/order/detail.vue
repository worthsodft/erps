		...
		<block v-if="from">
			...
			<view v-if="take_type == 0" class="confirm-item address" @click="showStationPopup">
				<view class="address-info" v-if="station && station.gid">
					{{station.title}}
					<text>{{'\n'}}</text>
					{{station.province?station.province+", ":''}}{{station.city?item.city+', ':''}} {{station.district?station.district+', ':''}}
			...
			<view v-else-if="take_type == 1" class="confirm-item address">
		...
		<block v-else-if="orderInfo.sn">
			...
			<view v-if="station && station.gid" class="confirm-item address">
				<view class="address-info">
					{{station.title}}
					<text>{{'\n'}}</text>
					{{station.province?station.province+", ":''}}{{station.city?item.city+', ':''}} <text class="color-green">{{station.district?station.district+', ':''}}</text>
					{{station.street?station.street+', ':''}} {{station.detail}}
					<text>{{'\n'}}</text>
					{{station.link_name}} {{station.link_phone}}
				</view>
			...
			<view v-else-if="address && address.gid" class="confirm-item address">
				<view class="address-info">
					{{address.province?address.province+", ":''}}{{address.city?address.city+', ':''}} <text class="color-green">{{address.district?address.district+', ':''}}</text>
					{{address.street?address.street+', ':''}} {{address.detail}}
					<text>{{'\n'}}</text>
					{{address.link_name}}，{{address.link_phone}}
				</view>
...
<!-- 		<view class="v-pb-10 v-pl-20 v-bb-3 v-mt-20">
		...
		<view class="confirm-item" v-if="remark">
			<view class="v-p-20 v-b-r-10">
				<text class="v-bold">订单备注：</text>{{remark}}
			</view>
		</view>
		...
		<!-- 操作栏 -->
		<v-order-confirm-bar2 v-if="orderInfo.sn && orderInfo.take_type == 1 && orderInfo.status == 1 && orderInfo.deliver_status == 1" :money="orderInfo.pay_amount||''" :number="orderInfo.goods_total" action="deliver" @deliver="deliver"/>
		<v-order-confirm-bar2 v-else :money="orderInfo.pay_amount||''" :number="orderInfo.goods_total" action="navto" @navto="back"/>
		<!-- 各种弹窗 -->
		...
		<v-deliver-images ref="deliverImagesRef" :sn="orderInfo.sn" :images="images" @success="deliverSuccess"></v-deliver-images>
	</view>
</template>
...
<script setup>
	...
	// 支付方式
	let payTypes = ref({weixin:"微信",yue: "余额"})
	let pay_type = ref("")
